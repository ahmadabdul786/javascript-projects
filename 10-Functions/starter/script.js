...
*/
const poll = {
  question: 'what is your favourite promgraming language',
  options: ['0:Javascript', '1:Python', '2:rust', '3:C++'],
  answers: new Array(4).fill(0),

  registerNewAnswer() {
    const answer = Number(
      prompt(`${poll.question} || ${poll.options.join('()=>')}`)
    );
    console.log(answer);
    typeof answer === 'number' && answer < 4 && this.answers[answer]++;

    //console.log(this.answers);
    this.displayResults();
  },
  displayResults(type = 'array', array = '') {
    //console.log('');
    if (type === 'array' && array === '') {
      console.log(this.answers);
